...
def _vault_entry(project):
    page_header(
        "实验记录与数据积累",
        "结构化沉淀KDP实验的工艺条件、过程现象、表征结果与失败信息，支持批次比较、异常追溯、数据质量评估及后续数据驱动建模。",
        "PROTECTED EXPERIMENT DATA",
    )

    section_title(
        "实验数据资产",
        "统一的数据结构使实验记录可用于历史检索、批次比较、统计分析，并为后续机器学习与条件优化提供连续数据基础",
    )
    st.markdown(
        """
**实验记录 → 历史比较 → 规律分析 → 数据建模 → 条件优化**

长期积累的结构化实验记录将形成项目专属实验知识库。
现阶段重点保证数据完整、统一、可追溯；数据规模与质量达到要求后，再逐步开展基线模型、
不确定性分析、主动学习与贝叶斯优化。
"""
    )

    st.caption(
        "数据保护：独立密码 · 会话隔离 · 自动锁定 · 加密备份 · 原始实验数据默认不进入外部AI。"
    )
    with st.expander("当前存储方式与数据保护说明", expanded=False):
        st.markdown(
            """
当前部署采用**加密实验保险库**管理真实实验数据，尚未启用独立持久化后端。
跨会话使用时，请下载 `.kdpvault` 加密备份，并在下次进入时重新导入。

该存储层后续可根据课题组要求迁移到内网、本地服务器或独立后端，
实验字段、项目关联和机器学习数据结构保持不变。
"""
        )
...
def _backup_security(project):
    stats = vault_stats()
    section_title("加密备份", "用于跨会话保存和恢复当前实验数据资产")
    ...
    st.info(
        "请将 `.kdpvault` 加密备份保存在受控位置。当前部署未启用独立持久化后端，"
        "浏览器会话结束、应用重启或重新部署前应更新备份。"
    )
    ...
    section_title("数据保护状态", "展示当前实验数据的访问、加密、备份与外部AI隔离状态")
